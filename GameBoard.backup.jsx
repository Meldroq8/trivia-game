...
function GameBoard({ gameState, setGameState }) {
  ...
  // Perfect scaling system to use all screen space without scrolling
  const getResponsiveStyles = () => {
    ...
    // Layout: 2 rows × 3 categories, each category is [BTN][CARD][BTN] × 3 rows
    const rows = 2
    const cols = 3
    const rowGap = 16
    const colGap = 12
    ...
    // Within each category group: [BTN][CARD][BTN] layout (3 equal columns, 3 rows)
    const innerColGap = 6
    const innerRowGap = 4
    const columnWidth = (categoryGroupWidth - (innerColGap * 2)) / 3 // 3 equal inner columns
    const buttonRowHeight = (categoryGroupHeight - (innerRowGap * 2)) / 3 // 3 inner rows

    // Button dimensions - fit within their column space
    const buttonWidth = columnWidth * 0.9
    const buttonHeight = buttonRowHeight * 0.85

    // Category card dimensions - same width as button columns, spans all 3 rows
    const cardWidth = columnWidth
    const cardHeight = categoryGroupHeight - (innerRowGap * 2)

    // Font sizes scale with button size
    const fontSize = Math.max(10, Math.min(28, buttonWidth * 0.2))

    // Smart card font scaling - calculate based on card width and text length
    const getCardFontSize = (categoryName, cardWidth) => {
      const maxFontSize = cardWidth * 0.15
      const minFontSize = 8
      const textLength = categoryName ? categoryName.length : 10

      // Scale down font size for longer text to prevent wrapping
      let scaledFontSize = maxFontSize
      if (textLength > 8) {
        scaledFontSize = maxFontSize * (8 / textLength)
      }

      return Math.max(minFontSize, Math.min(20, scaledFontSize))
    }
    ...
    return {
      ...
      buttonWidth: Math.max(40, buttonWidth),
        ...
        >
          {gameState.selectedCategories.slice(0, 6).map((categoryId, categoryIndex) => {
            ...
            return (
              <div
                key={categoryId}
                className="grid grid-rows-3 items-center justify-items-center"
                style={{
                  gridTemplateColumns: '1fr 1fr 1fr', // Ensure equal columns
                  gap: `${styles.innerRowGap}px ${styles.innerColGap}px`,
                  width: `${styles.categoryGroupWidth}px`,
                  height: `${styles.categoryGroupHeight}px`
                }}
              >
                {/* Row 1: [200] [Category] [200] */}
                <button
                  onClick={() => handleQuestionClick(categoryId, 200, 0)}
                  disabled={isPointValueUsed(categoryId, 200, 0)}
                  className={`border-2 font-bold rounded-lg shadow-md transition-all duration-200 ${
                ...
                {/* Category Card - spans all 3 rows */}
                <div
                  className="relative rounded-lg overflow-hidden border-2 border-gray-300 bg-gradient-to-br from-gray-200 to-gray-400 row-span-3 flex flex-col justify-end"
                  style={{
                ...
                >
                  ...
                  {!category.imageUrl && category.image && (
                    <div className="absolute inset-0 flex items-center justify-center"
                         style={{ fontSize: `${styles.cardHeight * 0.3}px` }}>
                      {category.image}
                    </div>
                  )}
                  <div
                    className="bg-red-600 text-white px-2 py-2 text-center font-bold whitespace-nowrap overflow-hidden"
                    style={{
                      fontSize: `${styles.getCardFontSize(category.name, styles.cardWidth)}px`,
                ...
                {/* Row 2: [400] [Category continues] [400] */}
                <button
                ...
                {/* Row 3: [600] [Category continues] [600] */}
                <button
        ...
        <div className="flex justify-between items-center max-w-6xl mx-auto">
          ...
          <div className="flex items-center gap-2">
            ...
            <div className="flex items-center gap-1">
              <div className="w-6 h-6 bg-gray-200 border border-gray-300 rounded-md flex items-center justify-center text-xs text-gray-500">
                1
              </div>
              <div className="w-6 h-6 bg-gray-200 border border-gray-300 rounded-md flex items-center justify-center text-xs text-gray-500">
                2
