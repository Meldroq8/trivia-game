...
export class AuthService {
  ...
  static async updateGameStats(uid, gameData) {
    try {
      ...
      devLog('🔥 [SAVE DEBUG] Starting updateGameStats for user:', uid)
      ...
      devLog('🔥 [SAVE DEBUG] rebuildGameData completed, result:', gameDataForFirebase ? 'has data' : 'NULL')
      ...
      if (gameId) {
        ...
        try {
          ...
          devLog('🔥 [SAVE DEBUG] Could not read existing game (will create):', readError.code)
        }
        ...
        devLog('🔥 [SAVE DEBUG] About to write to Firestore with gameId:', gameId)
        try {
          await setDoc(gameRef, {
            ...documentToSave,
            createdAt: documentToSave.createdAt || new Date()
          }, { merge: true })
          devLog('🔥 [SAVE DEBUG] ✅ Firestore write SUCCESS for gameId:', gameId)
        } catch (writeError) {
          prodError('🔥 [SAVE DEBUG] ❌ Firestore write FAILED:', writeError.code, writeError.message)
          throw writeError
        }
      } else {
        // Fallback: no gameId - create with auto-generated ID
        devLog('🆕 Creating new game (no gameId)')
        devLog('🔥 [SAVE DEBUG] About to create new game (no gameId)')
        documentToSave.createdAt = new Date()
        try {
          const newDocRef = await addDoc(collection(db, 'games'), documentToSave)
          devLog('🔥 [SAVE DEBUG] ✅ New game created with ID:', newDocRef.id)
        } catch (writeError) {
          prodError('🔥 [SAVE DEBUG] ❌ New game create FAILED:', writeError.code, writeError.message)
          throw writeError
        }
      }
      ...
      prodError('🔥 [SAVE DEBUG] ❌ MAIN ERROR in updateGameStats:', error.code, error.message, error)
