...
function Admin() {
  ...
  const { isAdmin, user, isAuthenticated, loading, userProfile } = useAuth()
  ...
  }, [isAuthenticated, isAdmin, navigate, loading, user])
  ...
  // Show access denied message if not authenticated or not admin
...
function CategoriesManager() {
  ...
  const saveCategories = async (newCategories) => {
    ...
    try {
      ...
      // Update each category in Firebase
  ...
  const handleImageFileUpload = async (categoryId, file) => {
    ...
    try {
      ...
      const downloadURL = await ImageUploadService.uploadCategoryImage(file, categoryId)
...
🔥 جميع البيانات محفوظة في Firebase وستظهر في اللعبة فوراً!`
  ...
  const handleQuestionImageUpload = async (categoryId, questionIndex, file) => {
    ...
    try {
      ...
      const downloadURL = await ImageUploadService.uploadQuestionImage(file, `${categoryId}_${questionIndex}`)
      ...
      alert('فشل في رفع صورة السؤال: ' + error.message)
    } finally {
      // Clear loading state
  ...
  return (
    <div>
      ...
      <div className="bg-purple-50 p-6 rounded-xl">
        <p className="text-purple-800">سيتم تطوير هذا القسم لإعدادات اللعبة العامة.</p>
