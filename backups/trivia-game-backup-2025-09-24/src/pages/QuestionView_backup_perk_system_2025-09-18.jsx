...
function QuestionView({ gameState, setGameState }) {
  ...
  const handleScoreTeam = (teamKey) => {
    ...
    // Calculate points (apply double points if active)
    const basePoints = currentQuestion.points
    const finalPoints = doublePointsActive ? basePoints * 2 : basePoints

    console.log('💰 Base points:', basePoints, 'Final points:', finalPoints, 'Double active:', doublePointsActive)
      ...
      return {
        ...
        gameHistory: [
          ...prev.gameHistory,
          {
            ...
            doublePointsUsed: doublePointsActive,
  ...
  const handlePerkClick = (perkType, team) => {
    if (!currentQuestion) return // Only allow perks when question is visible
  ...
  const handlePerkConfirm = () => {
    ...
    if (type === 'double') {
      ...
      // Close modal immediately for double points
      setPerkModalOpen(false)
      setActivePerk({ type: null, team: null })
    }
    // For phone and search, the modal handles the timer
  }
  ...
  // Reset double points when question changes
  useEffect(() => {
    setDoublePointsActive(false)
  }, [currentQuestion])
        ...
        <div className="bg-white flex-1 flex">
          ...
          <div className="flex flex-1 h-full">
              ...
              <div className="flex-1 bg-red-600 text-white flex flex-col items-center justify-center" style={{
                ...
                <div className="flex flex-col items-center justify-center h-full">
                  ...
                  <div className="flex" style={{ gap: `${styles.buttonPadding * 0.25}px` }}>
                    <div
                      className="bg-white bg-opacity-20 rounded-full flex items-center justify-center cursor-pointer hover:bg-opacity-30 transition-all duration-200"
                      style={{
            ...
            <div className="flex-1 flex flex-col relative h-full" style={{ padding: `${Math.max(8, styles.buttonPadding * 0.75)}px` }}>
                    ...
                    {currentQuestion.question?.audioUrl && (
                      <div className="flex justify-center" style={{ padding: `${Math.max(1, styles.buttonPadding * 0.03125)}px ${Math.max(2, styles.buttonPadding * 0.0625)}px` }}>
                        <div style={{ maxWidth: '400px', width: '100%' }}>
                          <AudioPlayer src={currentQuestion.question.audioUrl} />
                        </div>
                      </div>
                    )}
              ...
              <button
                onClick={() => handlePerkClick('phone', 'team1')}
                className="absolute bg-red-600 hover:bg-red-700 text-white font-bold rounded-lg border-2 border-white z-10"
                style={{
                  left: `50%`,
                  top: `${Math.max(-15, -10) + 10}px`,
